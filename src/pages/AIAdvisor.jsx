...
export default function AIAdvisor() {
  ...
  function exportChat() {
    ...
    doc.setFontSize(18); doc.setTextColor(...primary);
    doc.text('IMPUNGA AI Conversation', 105, 20, { align: 'center' });
    doc.setFontSize(9); doc.setTextColor(150);
    doc.text(new Date().toLocaleString('en-GB'), 105, 27, { align: 'center' });

    let y = 40;
    messages.filter(m => m.role !== 'error').forEach(m => {
      if (y > 265) { doc.addPage(); y = 20; }
      doc.setFontSize(11); doc.setTextColor(...(m.role === 'user' ? [0, 0, 0] : primary));
      doc.text(m.role === 'user' ? 'You' : 'IMPUNGA AI', 14, y);
      y += 7;
      doc.setTextColor(0, 0, 0);
      y = renderMarkdownBlock(doc, m.content, { startY: y, leftMargin: 14, rightEdge: 196, pageBottom: 275 });
      y += 8;
    });
    ...
    doc.save(`impunga-chat-${Date.now()}.pdf`);
  }
