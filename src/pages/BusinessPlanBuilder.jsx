...
export default function BusinessPlanBuilder() {
  ...
  const { critiqueBusinessPlan, generateFullBusinessPlan, loading } = useGemini();
  ...
  async function handleGenerateAndDownload() {
    try {
      const aiPlan = await generateFullBusinessPlan({
        ...data, totalStartup, totalMonthly, monthlyRevenue, monthlyCostOfGoods, grossProfit, netProfit,
      });
      generatePDF(aiPlan);
      show('Business plan PDF downloaded!');
    } catch (err) {
      show(err.message || 'Failed to generate your business plan. Please try again.', 'error');
    }
  }

  function generatePDF(aiPlan) {
    const doc = new jsPDF();
    ...
    const block = (text) => {
      if (!text) return;
      y = renderMarkdownBlock(doc, text, { startY: y, leftMargin: 14, rightEdge: 196, pageBottom: 275 });
      y += 5;
    };

    section('EXECUTIVE SUMMARY');
    block(aiPlan.executiveSummary);
    section('COMPANY DESCRIPTION');
    block(aiPlan.companyDescription);
    section('MARKET ANALYSIS');
    block(aiPlan.marketAnalysis);
    section('PRODUCTS & SERVICES');
    block(aiPlan.productsAndServices);
    section('MARKETING & SALES STRATEGY');
    block(aiPlan.marketingAndSalesStrategy);
    section('OPERATIONS PLAN');
    block(aiPlan.operationsPlan);
    section('MANAGEMENT & ORGANIZATION');
    block(aiPlan.managementAndOrganization);
    section('FUNDING REQUEST');
    block(aiPlan.fundingRequest);
